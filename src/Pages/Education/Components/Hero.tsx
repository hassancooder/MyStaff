...
const Hero: React.FC = () => {
  ...
  return (
    <section
      ...
      style={{
        backgroundImage: "url('./education-hero-bg.webp')",
      }}
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-20 mt-52">
        <div className="max-w-2xl flex flex-col" data-aos="fade-right">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-semibold mb-2 leading-tight">
            A critical tool to support {isWideScreen ? <br /> : " "} frontline
            governance
          </h2>
          <p className="text-base sm:text-lg text-white font-medium mb-8">
            An award-winning AI driven platform redefining
            {isWideScreen ? <br /> : " "}
            the way documents are managed and distributed
            {isWideScreen ? <br /> : " "}
          ...
          <Link
            to="/contact"
            className="bg-blue-700 text-white text-medium max-w-[275px] text-[15px] font-medium px-2 py-2 rounded-md transition-all duration-300 transform hover:scale-105"
          >
